  ...
  LOOP
    ...
    EXECUTE format(
      ...
      SELECT
        ...
        (ARRAY['18-24', '25-34', '35-44', '45-54', '55+'])[(mod(idx, 5)) + 1],
        (ARRAY['Female', 'Male', 'All'])[(mod(idx, 3)) + 1],
        (ARRAY['Helsinki', 'Tampere', 'Turku', 'Jyväskylä', 'Oulu'])[(mod(idx, 5)) + 1],
        (300 + (idx * 47))::numeric,
        'MONTHLY',
        (ARRAY['Meta Ads', 'Google Ads', 'LinkedIn Ads', 'Display Network'])[(mod(idx, 4)) + 1],
        ARRAY['https://picsum.photos/seed/demo-' || %L || '-' || idx || '/1200/630'],
        start_ts,
        end_ts,
        bucket = 'ongoing',
        (ARRAY['AD', 'PROMOTION', 'AWARENESS'])[(mod(idx, 3)) + 1],
        'https://demo.markku.app/tenant/' || %L || '/campaign/' || idx,
        'Learn more',
        CASE WHEN mod(idx, 2) = 0 THEN 'Leevi Admin' ELSE 'Mikko Manager' END
